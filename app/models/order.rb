class Order
  ...
  with_options presence: true do
    validates :user_id
    validates :item_id
    validates :postal_code, format: { with: /\A\d{3}-\d{4}\z/, message: "半角数字と前3桁、'-'、後4桁を使用してください" }
    validates :prefecture_id, numericality: { other_than: 1, message: "can't be blank" }
    validates :municipalities
    validates :house_number
    validates :phone_number, presence: true, format: { with: /\A\d{10,11}\z/, message: '半角数字、10桁もしくは11桁を使用してください' }
    validates :token
  end
